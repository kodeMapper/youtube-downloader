...
def download_video(url, output_dir):
    ...
    formats = ['best[height<=480]', 'best[height<=720]', 'best', 'worst']
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (infoOnly) {
      try {
        // Use YouTube oEmbed API for basic info (simpler and more reliable on Vercel)
        ...
        // Fallback to Python script if oembed fails
        const tempId = uuidv4();
...
def install_package(package_name):
    try:
        import subprocess
        subprocess.check_call([sys.executable, "-m", "pip", "install", package_name, "--quiet", "--user"])
        return True
    except:
        return False
...
`;
        ...
        const { stdout } = await execAsync(`python3 "${pythonInfoPath}" || python "${pythonInfoPath}"`, {
    ...
    // Create temporary directory for downloads (use /tmp for better Vercel compatibility)
    const tempId = uuidv4();
    const tempDir = path.join('/tmp', tempId);
